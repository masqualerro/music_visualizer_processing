...
color bgColor = color(#000000);
color textColor = color(#718689);
...
void draw() {
  ...
  if (darkClick == true) { //color mode changes
    ...
    bgColor = color(0);
    pbbgColor = color(0);
    pbtextColor = color(#718689);
    addtextColor = color(#718689);
    buttonColor = color(#718689);
  }
  ...
  if (extraClick == true) {
    ...
    bgColor = color(#D9D0D0);
    pbbgColor = color(#D9D0D0);
    pbtextColor = color(#897187);
    addtextColor = color(#897187);
    buttonColor = color(#897187);
  }
...
void mousePressed() {//same thing as above but for the Clickable Button class instances
...
void ellipse_visualizer(int x, int y) {
...
void rect_visualizer(int x, int y) {
  rectMode(CENTER);
...
//written by ALESSANDRA
void pageSetup() { //creating all of the variables from page builder class - called in setup thru this custom function (makes setup less crowded)
...
void textSetup() {//calling AddText variables 
...
//written by ALESSANDRA
void home() {//home page custom function - didn't use Pagebuilder bc dif. format
  background(bgColor);
  goBack.display();
  green.display();
  close.display();
...
void about() {//about page custom function called in draw
  background(bgColor);
  fill(pbtextColor);
  textSize(35);
  textAlign(LEFT, LEFT);
